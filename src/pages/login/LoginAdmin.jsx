...
export const LoginAdmin = () => {
  ...
  const submitHandler = async (data) => {
    ...
    try {
      ...
      const response = await axios.post(SERVER_URL + "/api/auth/admin-login", {
        email,
        password,
      ...
      if (response.status === 200) {
        ...
        console.error("Invalid Email or Password"); // Set an error message
      }
    } catch (error) {
      toast.update(id, {
      ...
      <div className="w-full min-h-screen flex items-center justify-center flex-col lg:flex-row bg-[#f3f4f6]">
        <div className="w-full md:w-auto flex gap-0 md:gap-40 flex-col md:flex-row items-center justify-center">
          ...
          <div className="w-full md:w-1/3 p-4 md:p-1 flex flex-col justify-center items-center">
            ...
            >
              <div className="">
                <p className="text-[#20409A] text-3xl font-bold text-center">Account Admin Only!</p>
              ...
              <div className="flex flex-col gap-y-5">
                ...
                <div className="flex justify-between">
                  <a
                    href="/forgetpassword"
                    className={clsx("text-sm text-gray-500 hover:underline cursor-pointer", `hover:text-[${themeColors[0]}]`)}
                  >
                    Forget Password?
                  </a>
                  <a
                ...
                <Button
                  type="submit"
                  label="Submit"
                  className={clsx("w-full h-10 hover:bg-green-800 text-white rounded-full", `bg-[${themeColors[0]}]`)}
                />
                {/* <span
                  onClick={() => addClick()}
                  className={clsx(
                    "flex items-center justify-center text-sm text-gray-500 hover:underline cursor-pointer",
                    `hover:text-[${themeColors[0]}]`
                  )}
                >
                  Create New Account?
                </span> */}
