...
export const ProjectDashView = ({ gridData, customDateFormat, usedCurrency, updatedCostData, updatedTaskData }) => {
  const stats = [
    {
      _id: "1",
      label: "Initiation",
      total: gridData?.filter((item) => item.ProjectStage === "Initiation").length || 0,
      icon: <MdFiberNew />,
    ...
    },
    {
      _id: "2",
      label: "Planning",
      total: gridData?.filter((item) => item.ProjectStage === "Planning").length || 0,
      icon: <MdRateReview />,
    ...
    },
    {
      _id: "3",
      label: "Execution",
      total: gridData?.filter((item) => item.ProjectStage === "Execution").length || 0,
      icon: <GiProgression />,
    ...
    },
    {
      _id: "4",
      label: "Monitoring",
      total: gridData?.filter((item) => item.ProjectStage === "Monitoring").length || 0,
      icon: <PiPulseFill />,
      footer: "implimenting",
    },
    {
      _id: "5",
      label: "Closing",
      total: gridData?.filter((item) => item.ProjectStage === "Closing").length || 0,
      icon: <FaCheckSquare />,
    ...
    return (
      <div className="w-full h-fit bg-white px-5 py-3 shadow-md rounded-md gap-5 flex flex-col justify-between text-center">
        ...
        <div className="h-full flex flex-row items-center justify-center gap-2">
          <span className={clsx("px-2 text-5xl font-bold", `text-[${themeColors[0]}]`)}>{count}</span>
          <div className={clsx("min-w-16 h-16 rounded-full flex items-center justify-center text-3xl md:text-4xl", getStatusColor(label))}>
            {icon}
          </div>
        </div>
        <div>
    ...
    <div className="flex flex-col h-full py-0 gap-10">
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-5">
        {stats.map(({ icon, label, total, footer }, index) => (
          <Card key={index} icon={icon} label={label} count={total} footer={footer} />
        ))}
      </div>
      <div className="grid grid-cols-1 2xl:grid-cols-3 gap-5">
        <PercentageCompleteChart updatedTaskData={updatedTaskData} />
        <ProjectCostChart updatedProjectData={gridData} updatedCostData={updatedCostData} usedCurrency={usedCurrency} />
      </div>
