...
export const AddDebt = ({ open, setOpen, recordData }) => {
  ...
  useEffect(() => {
    if (recordData?.id) {
      ...
      setValue('LoanBalance', recordData.LoanBalance ? Number(recordData.LoanBalance).toFixed(2) : 0);
    ...
    axios
      ...
      .then(({ data }) => {
        queryClient.setQueryData(['debts'], (prev) =>
          prev.map((debt) => (debt.id === numericSelectedID ? { ...debt, ...data } : debt)),
        );
        setIsLoading(() => false);
        setOpen(false);
      })
