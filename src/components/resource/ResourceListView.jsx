...
import { getFormattedDate, getUser } from '../../utils/nmrw.function';
    ...
    <tr className='border border-gray-300 text-sm xl:text-[16px] hover:bg-gray-400/10 text-left'>
      ...
      <td className='min-w-fit whitespace-nowrap p-2 border-l border-gray-200'>
        <span className='flex items-center justify-left gap-2 text-center'>
          {record?.updatedAt ? getFormattedDate(user, record?.updatedAt) : getFormattedDate(user, record?.createdAt)}
        </span>
      </td>
      <td className='min-w-fit whitespace-nowrap p-2 border-l border-gray-200'>
        <span className='flex items-center justify-left gap-2 text-center'>
          {record?.updatedById
            ? getUser(record?.updatedById, users)?.FullName
            : getUser(record?.createdById, users)?.FullName}
        </span>
      </td>
    </tr>
  );
    ...
    <>
      {gridData?.length > -1 && (
        <div className='w-full h-fit bg-white mt-4 shadow-md rounded-xl'>
          <div className='flex items-center justify-between gap-5 px-5 py-3 uppercase font-bold w-full bg-primary text-white rounded-xl'>
