...
const Sidebar = () => {
  ...
  const [sidebarData, setSideBarData] = useState(
    isAdmin ? SidebarLinks : SidebarLinks.slice(0, 1)
  );
  ...
  useEffect(() => {
    if (sidebarData?.length && !hasFin)
      setSideBarData((old) => old.filter((item) => item.label !== "Costs"));
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={
              el.sub?.length ? "text-[#27708A] hover:text-[#00ABBD]" : "hidden"
            }
          >
            <IoMdArrowDropright />
          </div>
          ...
          {el.sub?.map((item, index) => (
            <div
              className={`w-full text-md leading-6 px-3 py-1 rounded-full hover:bg-[#2564ed15]  ${
                parseInt(tab) === index && title === name
                  ? "md:text-white lg:text-[#27708A] bg-[#2564ed2d] "
                  : "text-[#A7A7A7]"
              }`}
              key={index}
            ...
            >
              <NavLink
                to={item.link + `?name=${title}&tab=${index}`}
      ...
      <div className="flex justify-between">
        <h1 className="flex gap-1 items-center mb-3">
          <img
            src={Logo}
            alt="logo"
            className="h-8 md:h-5 lg:h-8 px-3 md-px-0 lg:px-3"
          />
        </h1>
        <button
