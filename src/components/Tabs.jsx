...
export function Tabs2({ selected, tabs, children }) {
  const [_, setSearchParams] = useSearchParams();
        ...
        <Tab.List className="flex space-x-6 rounded-xl p-1">
          {tabs.map((tab, index) => (
            <Tab
              ...
              className={({ selected }) =>
                classNames(
                  'w-fit flex items-center outline-none gap-2 px-3 py-2.5 text-base font-medium leading-5 rounded-lg shadow-md shadow-slate-600 hover:text-green-500',

                  !selected
                    ? 'text-black bg-gray-200  border-t border-black'
