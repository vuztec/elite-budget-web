...
export const AddAdministrator = ({ open, setOpen, recordData, itemID, type, socket, admins, query }) => {
  ...
  const resources = useMemo(() => {
    return allresources?.filter((resource) => !admins?.includes(resource.value));
  }, [allresources]);
  ...
  const handleOnSubmit = async (data) => {
    ...
    axios
      .post(SERVER_URL + "/api/general/admin", { data, itemID })
      .then(({ data }) => {
      ...
      <ModalWrapper open={open} setOpen={setOpen}>
        <form onSubmit={handleSubmit(handleOnSubmit)} className="">
          <Dialog.Title as="h2" className="text-base font-bold leading-6 text-gray-900 mb-4">
            {recordData ? "UPDATE ADMINISTRATOR" : "ADD NEW ADMINISTRATOR"}
          </Dialog.Title>
          <div className="h-72 mt-2 flex flex-col gap-6 overflow-y-scroll bg-scroll">
            <Controller
              name="AssignedTo"
              control={control}
              render={({ field }) => <CustomSelect {...field} options={resources} label="Administrators" />}
            />
