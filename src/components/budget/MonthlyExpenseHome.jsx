...
const MonthlyExpenseHome = ({ expenseGridData, incomeGridData, maingoals, owner }) => {
        ...
        <table className="w-[99%]">
          <tbody>
            <tr className="border border-gray-300 text-left p-2">
              <td className="px-2 py-2 border-r w-2/3 border-gray-300">Total Annual Expenses</td>
              <td className="px-2 font-bold">{getYearlyBudgetTotal(user, expenseGridData)}</td>
            </tr>
            <tr className="border border-gray-300 text-left p-2">
              <td className="px-2 py-2 border-r w-2/3 border-gray-300">Percentage of Annual Net Income</td>
              <td className="px-2  font-bold">
                {owner === 'Joint' ? '' : getActualGoalYearly(incomeGridData, expenseGridData, 'Expenses')}%
              </td>
            </tr>
            <tr className="border border-gray-300 text-left p-2">
              <td className="px-2 py-2 border-r w-2/3 border-gray-300">Goal</td>
              <td className="px-2 font-bold">{owner === 'Joint' ? '' : getBudgetGoal(maingoals, 'Expenses')}%</td>
            </tr>
            <tr className="border border-gray-300 text-left p-2">
              <td className="px-2 py-2 border-r w-2/3 border-gray-300">
                {owner === 'Joint' ? 'Over/Under' : getLabel(incomeGridData, expenseGridData, 'Expenses', maingoals)}
              </td>
              <td className="px-2 font-bold">
                {owner === 'Joint' ? '' : getDifference(incomeGridData, expenseGridData, 'Expenses', maingoals, user)}
              </td>
            </tr>
          </tbody>
        </table>
