            ...
            <Popover.Panel className="absolute left-1/2 z-10 mt-3 w-80 max-w-sm -translate-x-1/2 transform px-4 sm:px-0 ">
              <div className="flex items-center gap-4 rounded-lg shadow-lg bg-white p-8">
                <div className="w-16 h-16 bg-[#20409A] rounded-full text-white flex items-center justify-center text-2xl ">
                  <span className="text-center font-bold">{getInitials(user?.resourcesdb?.FullName)}</span>
                </div>
                <div className="flex flex-col gap-y-1">
                  <p className="text-black text-xl font-bold">{user?.resourcesdb?.FullName}</p>
                  <span className="text-base text-gray-500 italic">{user?.resourcesdb?.Designation}</span>
                  <div className="flex gap-3 items-center text-left">
                    <MdEmail className={"text-sm text-primary"} />
                    <span className="text-blue-500">{user?.resourcesdb?.Email}</span>
                  </div>
                  <div className="flex gap-3 items-center text-left">
                    <FaPhoneSquare className="text-sm text-primary" />
                    <span className="text-gray-500">{user?.resourcesdb?.Telephone}</span>
                  </div>
                </div>
              </div>
            </Popover.Panel>
